        ...
        <div class="row mb-5">
            <div class="col-md-12">
             ...
             <div class="d-flex align-items-center justify-content-between">
                <h4>Partners</h4>
                <a href="{{ route('createPartner') }}" class="admin-head-btn">Create Partner</a>
             </div>
                ...
                <table id="blogList" class="table table-striped table-bordered" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($partners as $key => $partner)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td><img src="{{ asset('uploads/partner-images') }}/{{ $partner->image }}"
                                    class="img-fluid p-4 " alt=""></td>
                                <td>{{$partner->title}}</td>
                                <td>{{$partner->slug}}</td>
                                <td>{{$partner->desc}}</td>
                                ...
                                <td>
                                    <div class="blog-list-actions">
                                        <a href="{{ route('updatePartner', ['id' => $partner->id]) }}" class="blog-list-edit"><i class="fa-solid fa-pen-to-square"></i></a>
                                        {{-- <a href="#" class="blog-list-del"><i class="fa-solid fa-trash-can"></i></a> --}}
                                        <form method="POST" action="{{ route('deletePartner', ['id' => $partner->id]) }}">
                                            @csrf
